...
public interface UtenteRepository extends IBaseRepository<Utente> {
	...
	@Query("SELECT u FROM Utente u left join fetch u.sessione s left join fetch u.ruoli where s.tokenDiAutenticazione = ?1")
	Optional<Utente> findUtenteWithValidSessionFromToken(String token);
}
